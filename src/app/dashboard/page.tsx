        ...
        <div className="min-h-screen bg-background pb-24">
            <header className="sticky top-0 z-30 flex items-center justify-between border-b border-card-border bg-background/80 px-6 py-4 backdrop-blur-md">
                ...
                <div className="h-10 w-10 overflow-hidden rounded-full border border-card-border glass">
                    <div className="flex h-full w-full items-center justify-center bg-accent/10 text-xs font-bold text-accent uppercase">
                        D
                    </div>
                </div>
            </header>
            ...
            <main className="mx-auto max-w-lg px-6 pt-10">
                    ...
                    {workouts.map((workout, index) => (
                        ...
                        >
                            <div className="relative z-10 flex flex-col gap-2">
                                ...
                                <h3 className="text-3xl font-black tracking-tighter text-foreground group-hover:text-accent transition-colors">
                                    {workout.name}
                                </h3>
                            ...
                            <div className="mt-6 flex items-center justify-between">
                                <div className="flex -space-x-2">
                                    {[1, 2, 3].map((i) => (
                                        <div key={i} className="h-6 w-6 rounded-full border border-background bg-card-border glass" />
                                    ))}
                                    <span className="pl-4 text-[10px] font-bold text-muted uppercase tracking-tighter">
                                        +12k Others
                                    </span>
                                </div>
                ...
                <div className="glass mt-12 rounded-3xl p-6 neon-shadow animate-in fade-in slide-in-from-bottom-12 duration-1000">
                    <h3 className="text-sm font-bold tracking-widest text-muted uppercase">Your Stats</h3>
                    <div className="mt-6 grid grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            <span className="text-3xl font-black tracking-tighter text-foreground">12</span>
                            <span className="text-[10px] font-bold text-muted uppercase">Sessions</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="text-3xl font-black tracking-tighter text-accent">750kg</span>
                            <span className="text-[10px] font-bold text-muted uppercase">Total Lifted</span>
                        </div>
                    </div>
                </div>
            </main>
            ...
            <nav className="fixed bottom-6 left-1/2 z-40 w-full max-w-sm -translate-x-1/2 px-6">
                <div className="flex items-center justify-around rounded-full bg-background/80 py-3 backdrop-blur-xl border border-card-border shadow-2xl glass">
                    ...
                    <div className="flex flex-col items-center gap-1 text-muted transition-colors hover:text-accent p-2">
                        <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
