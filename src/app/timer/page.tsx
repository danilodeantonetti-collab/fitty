            ...
            <main className="mx-auto max-w-lg px-6 pt-8 space-y-8">
                ...
                <div className="flex justify-center">
                    <div className="relative">
                        <svg width="280" height="280" className="-rotate-90">
                            ...
                            <circle cx="140" cy="140" r="120" fill="none" stroke={phase === "REST" ? "#3b82f6" : "#e2ff00"} strokeWidth="8" strokeLinecap="round"
                ...
                <div className="flex justify-center">
                    {phase === "IDLE" ? (
                        ...
                        <button onClick={stop} className="rounded-2xl border border-red-500/30 bg-red-500/10 px-12 py-3 text-lg font-bold text-red-400 hover:bg-red-500/20 active:scale-95">
                            Stop
                        </button>
                    )}
                </div>
                ...
                {phase === "IDLE" && (
                    <>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {presets.map((p) => (
                                    <button key={p.label} onClick={() => { setWorkTime(p.work); setRestTime(p.rest); }}
                                        className={`rounded-2xl p-4 text-left border transition-all hover:scale-[1.02] active:scale-[0.98] ${workTime === p.work && restTime === p.rest ? 'border-accent bg-accent/10' : 'border-card-border bg-card-border/20'}`}>
                        ...
                        <div className="rounded-3xl p-6 space-y-6 border border-card-border bg-card-border/20">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <div className="flex justify-between mb-2"><span className="text-xs font-bold text-foreground">Arbeitszeit</span><span className="text-xs font-black text-accent">{formatTime(workTime)}</span></div>
                                    <input type="range" min="10" max="300" step="5" value={workTime} onChange={(e) => setWorkTime(Number(e.target.value))} className="w-full h-2 rounded-full appearance-none bg-card-border cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-accent" />
                                </div>
                                <div>
                                    <div className="flex justify-between mb-2"><span className="text-xs font-bold text-foreground">Pausenzeit</span><span className="text-xs font-black text-blue-400">{formatTime(restTime)}</span></div>
                                    <input type="range" min="10" max="300" step="5" value={restTime} onChange={(e) => setRestTime(Number(e.target.value))} className="w-full h-2 rounded-full appearance-none bg-card-border cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-blue-400" />
                                </div>
                            </div>
                        </div>
                    </>
