...
export default function CustomWorkoutSession() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (error || !plan) { router.push("/dashboard"); return; }

            setPlanName(plan.name);
            const exList = plan.exercises as ExerciseEntry[];
            setExercises(exList);
    ...
    const addSet = (name: string) =>
        setLogs(p => ({ ...p, [name]: [...(p[name] || []), { reps: "", weight: "" }] }));

    const addWarmup = (name: string) =>
        setLogs(p => ({ ...p, [name]: [{ reps: "", weight: "", isWarmup: true }, ...(p[name] || [])] }));

    const updateSet = (name: string, idx: number, field: "weight" | "reps", value: string) =>
        setLogs(p => {
            const n = { ...p };
            n[name] = [...n[name]];
            n[name][idx] = { ...n[name][idx], [field]: value };
            return n;
        });

    const removeSet = (name: string, idx: number) =>
        setLogs(p => ({ ...p, [name]: p[name].filter((_, i) => i !== idx) }));
    ...
    const handleFinish = async () => {
        try {
            ...
            // Create session without workout_id (custom plan)
            ...
            (prevSets || []).forEach((s: any) => {
                if (s.exercise_name && s.weight != null) {
                    if (!allTimeBest[s.exercise_name] || s.weight > allTimeBest[s.exercise_name]) {
                        allTimeBest[s.exercise_name] = s.weight;
                    }
                }
            });
            ...
            exercises.forEach(ex => {
                ...
                const maxNew = workingSets.length
                    ? Math.max(...workingSets.map(s => parseFloat(s.weight)))
                    : 0;
                if (maxNew > 0 && maxNew > (allTimeBest[ex.name] || 0)) {
                    prs.push({ name: ex.name, weight: maxNew });
                }
    ...
    if (loading) return (
        <div className="flex h-screen items-center justify-center bg-black text-accent font-bold">
            Lade...
        </div>
    );
                    ...
                    {newPRs.map(pr => (
                        <div key={pr.name} className="rounded-2xl border border-accent bg-accent/10 p-4">
                            ...
                            <p className="text-3xl font-black text-foreground mt-1">
                                {pr.weight} <span className="text-base text-muted">kg</span>
                            </p>
                        </div>
                    ))}
                </div>
        ...
        <div className="min-h-screen bg-background pb-32">
            <header className="sticky top-0 z-30 flex flex-col border-b border-card-border bg-background px-6 py-4 gap-3">
                <div className="flex w-full items-center justify-between">
                    ...
                    <h1 className="text-lg font-black tracking-tighter text-foreground uppercase truncate max-w-[60%] text-center">
                        {planName}
                    </h1>
                            ...
                            <div key={ex.name}>
                                <div className="flex items-start justify-between border-b border-accent/20 pb-2">
                                    ...
                                    <div className="flex gap-2 flex-shrink-0">
                                        <button onClick={() => addWarmup(ex.name)}
                                            className="rounded-lg bg-card-border/40 px-2 py-1 text-[9px] font-bold text-muted uppercase hover:text-foreground">
                                            + Warm
                                        </button>
                                        <button onClick={() => addSet(ex.name)}
                                            className="rounded-lg bg-card-border px-2 py-1 text-[9px] font-bold text-accent uppercase hover:bg-accent/10">
                                            + Set
                                        </button>
                                    </div>
                                </div>
                                <div className="mt-4 space-y-3">
                                    {(logs[ex.name] || []).map((set, idx) => (
                                        <div key={idx} className="flex items-center gap-3">
                                            ...
                                            <button onClick={() => removeSet(ex.name, idx)}
