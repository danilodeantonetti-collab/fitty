...
export default function WorkoutSession() {
        ...
        const loadWorkoutData = async () => {
            ...
            try {
                ...
                if (user) {
                    ...
                    if (lastSession) {
                        const { data: sets } = await supabase.from('sets').select('exercise_id, weight, reps').eq('session_id', lastSession.id).order('order', { ascending: true });
                        if (sets) {
                            const perfMap: Record<string, { weight: string, reps: string }> = {};
                            const setsMap: Record<string, SetLog[]> = {};
                            sets.forEach(s => {
                                if (!perfMap[s.exercise_id]) perfMap[s.exercise_id] = { weight: s.weight.toString(), reps: s.reps.toString() };
                                if (!setsMap[s.exercise_id]) setsMap[s.exercise_id] = [];
                                setsMap[s.exercise_id].push({ weight: s.weight.toString(), reps: s.reps.toString() });
                            });
    ...
    const addWarmup = (exId: string) => setLogs(p => ({ ...p, [exId]: [{ reps: "", weight: "", isWarmup: true }, ...(p[exId] || [])] }));
    const updateSet = (exId: string, idx: number, field: keyof SetLog, value: string) => {
        const n = { ...logs }; n[exId] = [...n[exId]]; n[exId][idx] = { ...n[exId][idx], [field]: value }; setLogs(n);
    };
    ...
    if (showPRModal) return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-background px-6 text-center">
            <div className="animate-in zoom-in duration-500 space-y-6 max-w-sm w-full">
                ...
                <button onClick={() => router.push("/dashboard")} className="btn-primary w-full text-lg mt-4">Weiter 💪</button>
            </div>
        </div>
    );

    const hasLastSession = Object.keys(lastSets).length > 0;
                                ...
                                <div className="mt-4 space-y-3">
                                    {(logs[ex.id] || []).map((set, idx) => (
                                        <div key={idx} className="flex items-center gap-3">
                                            ...
                                            <button onClick={() => removeSet(ex.id, idx)} className="text-muted hover:text-red-400 transition-colors p-1">
